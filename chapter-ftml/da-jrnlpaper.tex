...
The contents of this section explain the subsequent analysis explanding on the approach, methodology, and results of the initial investigations using \gls{ml} to infer the \gls{sir} of a single radio link using situational awareness information from a camera system as published in~\cite{CandellISIE2019.Conf}.  In this further investigation, the performance of the \gls{ml} algorithms themselves were investigated to determine optimal selection given the particular use case. Publication is forthcoming~\cite{Candell2020.Jrnl.Access}.
...
\begin{figure}[!ht]
	...
	\caption{The \gls{mse} performance for jamming setting b/g-Router of \gls{xgbm} algorithm against $M$ including individual features impact.}
