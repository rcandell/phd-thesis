...
\begin{figure}[!ht]
	...
	\caption{A time-series sample~\protect\subref{ftml-conf:fig:timeseries-zplot} of a single iteration of the measured z-axis probe position and~\protect\subref{ftml-conf:fig:timeseriesfeatures} the corresponding \gls{reef} trajectory model used for feature extraction.}
	\label{ftml-conf:fig:timeseries-and-model}
...
\begin{figure}[tbp]
 ...
 \caption{The performance of the random forest regression model against segment size, $M$.  As $M$ increases, bias is reduced but variance increases shown a tendency to over-fit as the training window is increased. }
