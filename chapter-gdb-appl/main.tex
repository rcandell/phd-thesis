...
%\title{A Collaborative work cell Testbed for Industrial Internet of Things Networking}
% More about a cyber-physical system over an Internet of Things use case
\title{A Graph Database Approach to Wireless IIoT Workcell Performance Evaluation}
% The major tone is that this work is part of the cyber-physical systems (CPS) studies conducted at the National Institute of Standards and Technology (NIST). CPS and the other well known technical paradigm, industrial Internet of Things (IIoT), serve as emerging trends in enterprise sectors' Industry 4.0 initiatives. 
%
%
...
%\author{ 
%		$\mbox{Yongkang Liu}$, %~\IEEEmembership{Member,~IEEE,}
%        $\mbox{Richard Candell}$, %~\IEEEmembership{Senior Member,~IEEE,}
%        $\mbox{Mohamed Kashef}$, %~\IEEEmembership{Member,~IEEE,}
%        and $\mbox{Karl Montgomery}$
%%        and $\mbox{Kang Lee?}$ % ,~\IEEEmembership{Fellow,~IEEE,}
%
%%\thanks{Manuscript received 5 January 2012; revised 15 May 2012.}
%\thanks{Y. Liu and M. Kashef are with Advanced Network Technologies Division, Information Technology Laboratory, National Institute of Standards and Technology, Gaithersburg, MD, USA 20899 (e-mail: \{yongkang.liu, mohamed.hany\}@nist.gov).}% <-this % stops a space
%\thanks{R. Candell and K. Montgomery are with Intelligent Systems Division, Engineering Laboratory, National Institute of Standards and Technology, Gaithersburg, MD, USA 20899 (e-mail: \{richard.candell, karl.montgomery\}@nist.gov).}% <-this % stops a space
%}


\author{\IEEEauthorblockN{
    Richard Candell\IEEEauthorrefmark{1},
    Mohamed Kashef\IEEEauthorrefmark{1},
    Yongkang Liu\IEEEauthorrefmark{1},
	Karl Montgomery\IEEEauthorrefmark{1},
	Sebti Foufou\IEEEauthorrefmark{2}}\\
	\IEEEauthorblockA{
	    \IEEEauthorrefmark{1}National Institute of Standards and Technology, Gaithersburg, Maryland, USA \\
		Email: \{richard.candell, mohamed.kashef, yongkang.liu, karl.montgomery\}@nist.gov \\
		\IEEEauthorrefmark{2} University of Burgundy, Dijon, France, Email: sfoufou@u-bourgogne.fr
}}


% The paper headers
%\markboth{IEEE TRANSACTIONS ON INDUSTRIAL INFORMATICS,~VOL.~XX, NO.~XX, MAY~2016}%
%{LIU \MakeLowercase{\textit{et al.}}: A Co-Simulation Framework for Industrial Wireless Networks and Process Control Systems}
...
%TO-DO: 1) author list; 2) blind review process?
	
%Being the atomic service unit in a plant, 
...
%	\includegraphics[width=\textwidth,trim=50 50 50 50, clip]{figures/database/arrows-schema.PNG}
	\includegraphics[width=\textwidth]{figures/database/graph_schema_0816.eps}
...
Relationships are edges within the graph that capture the informational interactions between nodes.  Relationships, like nodes, can contain labels and properties.  As shown in Fig.~\ref{fig:database:schema}, nodes are connected through defined relationships.  A subset of the relationships are defined as follows:
\begin{description}
...
\end{description}
Other relationships shown in the schema of Fig.~\ref{fig:database:schema} but not explained above are considered self-explanatory.
...
After populating the database with data captured from the trial run, the resulting realized schema is shown in Fig.~\ref{fig:real-schema}. The schema visualization is produced by invoking the command

\begin{lstlisting}
call db.schema.visualization()
\end{lstlisting} 
in Neo4j. It is important to note that a realized schema shows only one representation of each node and relationship whereas the intended pseudo-schema shown in Fig.~\ref{fig:database:schema} was developed to exemplify the relationships between types of nodes, labels, and relationships. Where label inheritance is employed, such as the case for different adapter types, relationships are reproduced; however, this is a result of the visualization tool rather than the schema itself.  Fig.~\ref{fig:real-schema} serves, therefore, to validate that the intended schema was indeed realized by the insertion of event data from the testbed. In the realized schema, inherited labels are shown as separate nodes.

\begin{figure}
    \centering
    \includegraphics[width=0.99\columnwidth]{figures/database/graph_schema.png}
...
\begin{figure}
	\centering
%	\includegraphics[width=\textwidth,trim=50 50 50 50, clip]{figures/database/arrows-schema.PNG}
	\includegraphics[width=0.45\textwidth]{figures/database/scatter_1.eps}
	\caption{Correlation between the message latency and the RSSI values by the sniffer.}
	\label{fig:database:scatter}
\end{figure}
